...
func main() {
	...
	for d.More() {
		...
		if err != nil {
			var errMissingField ErrMissingField

			if errors.Is(err, ErrInvalidID) {
				fmt.Printf("record %d: %+v error: invalid id: %s\n", count, emp, emp.ID)
			} else if errors.As(err, &errMissingField) {
				fmt.Printf("record %d: %+v error: missing field: %s\n", count, emp, errMissingField.Field)
			} else {
...
func ValidateEmployee(e Employee) error {
	...
	if len(e.FirstName) == 0 {
		return ErrMissingField{Field: "FirstName"}
	}
	if len(e.LastName) == 0 {
		return ErrMissingField{Field: "LastName"}
	}
	if len(e.Title) == 0 {
		return ErrMissingField{Field: "Title"}
	}
	return nil
}
